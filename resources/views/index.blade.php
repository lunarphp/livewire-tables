        ...
        <x-tables::support.modal>
            <div class="p-4 space-y-4">
                <x-hub::input.group for="savedSearchName"
                                    label="Name"
                                    :error="$errors->first('savedSearchName')">
                    <x-hub::input.text id="saveSearchName"
                                       wire:model="savedSearchName" />
                </x-hub::input.group>

                <x-tables::button type="button"
                                  wire:click="saveSearch">Save Search</x-tables::button>
            </div>
        </x-tables::support.modal>
    </div>
            ...
            <div class="p-4">
                <div class="flex items-center gap-2 sm:gap-4">
                    ...
                    @if ($this->hasSearchApplied)
                        <button x-on:click="savingSearch = true"
                ...
                @if (count($this->savedSearches))
                    <div class="flex items-center gap-4 mt-2">
                        @foreach ($this->savedSearches as $savedSearch)
                            <div
                                 class="flex items-stretch overflow-hidden text-gray-600 transition bg-white border border-gray-200 rounded-md hover:shadow-sm focus-within:border-blue-300 focus-within:ring focus-within:ring-blue-100">
                                <button wire:click="deleteSavedSearch({{ $savedSearch['key'] }})"
                                        class="px-2.5 py-2 transition border-r border-gray-200 hover:bg-gray-50 hover:text-red-700 focus:outline-none focus:bg-gray-100">
                                    <span class="sr-only">Delete Saved Search</span>
                                ...
                                <button wire:click="applySavedSearch({{ $savedSearch['key'] }})"
                                        class="px-3 py-2 text-xs font-medium tracking-tight transition hover:bg-gray-50 hover:text-gray-700 focus:outline-none focus:bg-gray-100">
                                    <span class="sr-only">Apply Saved Search</span>
                    ...
                    <tbody class="relative">
                        @foreach ($this->rows as $row)
                            <tr class="bg-white even:bg-gray-50"
                                ...
                                <x-tables::cell
                                                class="sticky inset-y-0 right-0 text-right {{ $loop->odd ? 'bg-white/90' : 'bg-gray-50/90' }}">
